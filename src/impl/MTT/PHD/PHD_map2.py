...
class PHD_map:
    ...
    def updateWithMeasurements(self, time):
        ...
        for l, z in enumerate(np.array(self.measurements[time]).T):
            ...
            phds_sum = 0
            gatings = 0
            start_index = len(self.phds)
            for j in range(Jk):
                if self.phds[j].inGating(z):
                    ...
                    gatings += 1
            # print("len phds: ", len(self.phds))
            # print("gatings: ", gatings)
            # print("l: ",l)
            for j in range(gatings):
                # print("index: ", start_index)
                self.phds[start_index + j].w = self.phds[start_index + j].w / (self.radarMap.lambd + phds_sum)
    ...
    def run(self):
        ...
        for t in range(self.radarMap.ndat):
            ...
            # self.update()
            # if (t % 2 == 0):
            # self.applyGating(self.measurements[t].T)
