...
\subsection{V3 -- GM-PHD with dynamic detection probability}
...
Till the frame no. 85, the number of displayed targets (blue line) and the number of targets in filter's queue (orange line) exactly copies the true number of target's line in Graph \ref{gr:\Ex-\Vs-\Set}. It is of course caused by the better object detector, which was able to detect semi-hidden targets. The incresing and decreasing number of targets after frame 85 is due to the targets being close to each other.
